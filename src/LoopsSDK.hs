...
validateEmail e =
  let bs = BS8.pack (T.unpack e)
   in when (not $ bs RE.=~ emailPattern) $
        throwIO $ ValidationError "'email' must be a valid email address."
...
newClient apiKey mRoot = do
  let root = ensureSlash $ maybe "https://app.loops.so/api/" id mRoot
...
findContact c mEmail mUserId apiKey = do
  when (mEmail /= Nothing && mUserId /= Nothing) $ throwIO $ ValidationError "Only one of 'email' or 'user_id' may be provided."
  when (mEmail == Nothing && mUserId == Nothing) $ throwIO $ ValidationError "You must provide either 'email' or 'user_id'."
...
getCustomProperties c list_ apiKey = do
  when (list_ `notElem` ["all", "custom"]) $ throwIO $ ValidationError "list must be 'all' or 'custom'."
...
sendEvent :: LoopsClient ->
  Text -> Maybe Text -> Maybe Text -> Maybe Object -> Maybe Object -> Maybe Object -> Maybe [(BS8.ByteString, BS8.ByteString)] -> Text -> IO Value
...
sendTransactionalEmail c email mHeaders apiKey =
  postJsonWithHeaders c "v1/transactional" (toJSON email) mHeaders apiKey
